...
- `/opt/declouding/config/services/<name>.toml` — root:root, mode **0644**, world-readable. Holds non-secret operational metadata: `schema_version`, `name`, `source`, `build`, `run` (incl. `mounts` paths), `routes`, `strategy`, `readiness`, `state`. Safe to inspect; future "git-mirror the non-secret config" idea stays cheap.
- `/opt/declouding/secrets/<name>/env.toml` — root:root, mode **0600**, in a directory at mode **0700**. Holds `schema_version` (must match config) + `env` map (the `env.sh` capture). M3 will add `secrets/<name>/files/` for secret file contents.
