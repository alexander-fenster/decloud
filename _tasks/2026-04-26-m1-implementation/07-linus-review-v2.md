...
3. **One thing missing:** the `_docs/operator/usage.md` §6 says "If validation passed but the actual `caddy reload` failed (rare)..." — but Raymond should also mention that if the operator has manually edited `/opt/decloud/config/caddy/Caddyfile` (out-of-band), `decloud caddy reload` will REGENERATE from the registry and OVERWRITE the operator's edits. This is the trap an operator falls into when they hand-edit the Caddyfile to fix something and then run `caddy reload` expecting it to honor their edits. **Suggestion for Raymond's §6:** add one sentence: "Note: `decloud caddy reload` regenerates the Caddyfile from the registry; any manual edits to `/opt/decloud/config/caddy/Caddyfile` are discarded. If you need to inject custom Caddy directives per service, that's M5 work and currently unsupported."
