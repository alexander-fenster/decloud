...
4. **Create the `/opt/decloud/` tree** — exact `mkdir -p` + `chmod` commands for `config/services/`, `config/jobs/`, `config/caddy/`, `secrets/` (mode 0700), `state/deploys/`, `logs/`. The bootstrap script in M2 will eventually automate this; for M1 the operator runs the commands by hand.
...
| `slog`-based structured logging | **YES — ship in M1.** | Tech-plan §9.3 spec is good. The `DECLOUD_LOG_TO_STDERR_ONLY=1` test escape hatch is mandatory so unit tests don't write to `/opt/decloud/logs/`. |
